package com.frogniche.legendfoxes.server.entity.variants.blaze_runt;
...
public class BlazeRuntEntity extends Monster implements GeoEntity {
    ...
    public static final EntityDataAccessor<Integer> TYPE = SynchedEntityData.defineId(BlazeRuntEntity.class, EntityDataSerializers.INT);
    ...
    public BlazeRuntEntity(EntityType<BlazeRuntEntity> type, Level world) {
        super(type, world);
    }
    ...
    protected BlazeRuntEntity(Level world) {
        this(LFEntityHandler.BLAZE_RUNT.get(), world);
    }
    ...
    public boolean doHurtTarget(Entity opfer) {
        if(super.doHurtTarget(opfer)){
            ...
            if (opfer instanceof LivingEntity) {
                ((LivingEntity)opfer).addEffect(new MobEffectInstance(MobEffects.HARM,100), this);
            }
        }
        return true;
    }

    private PlayState attackPredicate(AnimationState<BlazeRuntEntity> event) {
        if (this.swinging && event.getController().getAnimationState() == AnimationController.State.RUNNING) {
            event.getController().forceAnimationReset();
            event.getController().setAnimation(RawAnimation.begin().then("animation.blaze_runt.attack",
    ...
    private <T extends GeoAnimatable> PlayState predicate(AnimationState<T> tAnimationState) {
        if(tAnimationState.isMoving()) {
            tAnimationState.getController().setAnimation(RawAnimation.begin().then("animation.blaze_runt.run", Animation.LoopType.LOOP));
            return PlayState.CONTINUE;
        }

        tAnimationState.getController().setAnimation(RawAnimation.begin().then("animation.blaze_runt.idle", Animation.LoopType.LOOP));
        return PlayState.CONTINUE;
    }
    @Override
    ...
    public static enum Type {
        HORDE_OF_THE_HUNT(LegendFoxes.modLoc("textures/entity/variants/runts/blaze_runts/hunt_variant.png")),
        HORDE_OF_THE_BASTION(LegendFoxes.modLoc("textures/entity/variants/runts/blaze_runts/bastion_variant.png")),
        HORDE_OF_THE_SPORE(LegendFoxes.modLoc("textures/entity/variants/runts/blaze_runts/spore_variant.png"));
