...
public class KingPawsEntity extends Monster implements GeoEntity {
    ...
    private PlayState predicate(AnimationState<KingPawsEntity> devourerEntityAnimationState) {

        if (devourerEntityAnimationState.isMoving()) {
            devourerEntityAnimationState.getController().setAnimation(RawAnimation.begin().then("animation.king_paws.walk", Animation.LoopType.LOOP));
            return PlayState.CONTINUE;
        } else {
            devourerEntityAnimationState.getController().setAnimation(RawAnimation.begin().then("animation.king_paws.idle", Animation.LoopType.LOOP));
        }
        return PlayState.CONTINUE;
